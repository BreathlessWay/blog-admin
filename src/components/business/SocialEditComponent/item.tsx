...
const SocialEditItem: FC<ISocialEditItemPropType> = (props) => {
	const {
		isEditing, title, value, file, label = "", onUploadFile, onDeleteFile, onDeleteItem, onChangeInput, error = false, errorMsg, type, accept = ""
	} = props;
	return <>
		<Col span={2}>
		...
		<Col span={6}>
			<Input value={value} maxLength={13} disabled={!isEditing} onChange={onChangeInput}/>
		</Col>
		<Col span={5} offset={1}>
			<UploadFileComponent accept={accept} label={label} disabled={!isEditing} onUploadFile={onUploadFile}/>
		</Col>
		<Col span={10}>
